...
app.controller('CheckoutController', function($scope, $interval, Order, $httpParamSerializer) {
  ...
  $scope.pay_altcoins = function() {
    ...
    url = "https://shapeshift.io/shifty.html?destination=" + $scope.address + "&amount=" + $scope.order.satoshi/1.0e8 + "&output=BTC";
  ...
  if ( typeof $scope.address != 'undefined'){
      //Fetch Address and Status
      //$scope.order = data;
      $scope.order.address = $scope.address 
      //Listen on websocket for payment notification
      //After getting notification,  refresh page
      if($scope.order.status == -1){
        $scope.clock = $scope.timestamp - Math.floor(Date.now() / 1000) + totalTime; // Was an issue with order of vars. Dunno why. 
        ...
        //Websocket
        var ws = new ReconnectingWebSocket("wss://www.blockonomics.co/payment/" + $scope.order.address)// + "?timestamp=" + $scope.timestamp); ***** Bring Back Timestamp *****
        ws.onmessage = function (evt) {
          ws.close();
					$interval(function(){
					 //Redirect to order received page
					  	window.location = $scope.finish_order;
